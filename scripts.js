...
document.getElementById('vcfToTxtFileInput').addEventListener('change', function(event) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            const content = e.target.result;
            document.getElementById('txtFileContent').value = content;
        };
        reader.readAsText(file);
    }
});
...
document.getElementById('splitButton').addEventListener('click', function() {
    ...
    reader.onload = function(e) {
        const content = e.target.result;
        const contacts = content.split('END:VCARD').map(contact => contact.trim() + 'END:VCARD').filter(contact => contact.length > 10);
